/// fx's own mark, the `fx` glyph from fx.sh's nav, on `currentColor`. Copied
/// from the app's `AgentIcon` rather than imported — the site shares the app's
/// design language and none of its code.
///
/// Its native viewBox is kept (`166.241 0 155.861 156`): the coordinates are
/// the glyph's own, and rescaling to 24 to match OpenAI's buys nothing but
/// rounding. `fillRule="evenodd"` is load-bearing — the counter inside the
/// glyph is a hole cut by a second contour in the one path, which the default
/// `nonzero` fills in as a slightly wrong solid blob.
export function FxGlyph({ className }: { className?: string }) {
  return (
    <svg
