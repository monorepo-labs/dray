...
| `model_list.json` | The `model/list` answer, `includeHidden: true` so the two internal rows (`gpt-reserve`, `codex-auto-review`) are there to be dropped. Not a stdio capture — a response body, like `fixtures/pr_graphql.json` — and captured off **`codex-cli 0.154.0`**, newer than the runs above. What `harness/codex/models.rs` read: ids, ladders, `defaultReasoningEffort`, `inputModalities`. |
| `model_list_gpt6.json` | The same answer off **`codex-cli 0.156.1`**, the first to list `gpt-6-sol` and `gpt-6-luna`. Kept beside the older one rather than replacing it: `model_list.json` is now what a reader who has not updated Codex answers, and the picker must draw no 6 Sol row there. |
