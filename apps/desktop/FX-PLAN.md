...
| provider switch | — | Works in place, and **the session keeps it**. `set_config_option provider` answers ok, leaves `~/.fx/settings.json` byte-identical, moves the session onto that provider's own remembered model, and **persists onto fx's session record** — a `session/resume` in a fresh process comes back on it, not on what the file names (0.0.10, 2026-09-16, DRA-241, `provider_switch.jsonl`). Two earlier notes here were wrong in opposite directions: one had it writing the file, the next had it dying with the process. **The only thing that writes `provider` is `fx provider <name>`**, which also drops a pre-write snapshot into `~/.fx/backups` and rewrites the mode to `0600`. A live `fx acp` reads the file once and never re-reads it: settings edited underneath one are neither noticed nor reconciled |
...
- **Provider is a composer control an existing session may move, and it also writes fx's global setting.** `set_fx_provider` writes the `provider` field itself and forgets the list. The global write is not optional: the picker's list is the *active* provider's, so nothing else makes it follow. What moves the session is the model — see DRA-241 below.

  **Both the unverified halves of this are now measured** (0.0.10, 2026-09-15, DRA-223), and both went the other way to the guess. A cross-provider `set_config_option model` on a running session is **refused** with `-32602`, not honoured. And the in-session `provider` switch writes **nothing** to the settings file.

  **DRA-241 is what the refusal cost, and where it is answered** (0.0.10, 2026-09-16). That `-32602` was a `?` inside `fx::set_model`: it failed the send, and on a resume `init` killed the child over it, so a session whose global provider had moved underneath it could not be opened at all. `provider_move` now sends `set_config_option provider` ahead of the model wherever the two disagree — a model names its provider, so switching across is what picking it *meant* — and both the live send and the resume go through that one function. Which is also what lets the composer draw the provider row for an existing session, disabled while a turn is in flight. The `session/resume` half of the earlier note is retired: the switch persists, so a resumed session comes back on the provider it was moved to.
