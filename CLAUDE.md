...
Cache module-level per repo, fresh **120s** — longer than panel's 30s, because this feed mark rather than view being read. Refreshed on same turn-end edge panel use, so mark and pane land together. Failure silent (no `gh`, not GitHub repo, logged out) but **not destructive**: failed read keep last good answer and stamp *it*, so blip catching repo mid-refresh don't blank rows that were right. Stamp = half that matter — it what stop repo `gh` genuinely can't answer for from being asked every render. Empty map only where no previous answer to keep, so first read failing still read as "no marks" rather than leaving repo unstamped and re-asked forever. Undefined therefore mean both "no PR" and "couldn't ask". Same rule `usePullRequest` keep its own list by: failed refresh = refresh that failed, not PR going away.
