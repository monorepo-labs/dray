...
**Button arming confirm, not switch — switch was first draft and wrong with confirm on it.** Change want confirming, because it make app start downloading different build on own; switch that must be confirmed cannot move on click, and switch that not move read as broken. Button carry ask honestly: label name act (Turn on / Turn off), which also say which channel live. Confirm = disconnect row's shape exactly — Cancel + verb replace button, Cancel take focus unmounting button drop (keyboard user was on that very spot, and Cancel not verb so habitual double-Enter change nothing). Description stay put through it — consequence sentence tried and taken back out.
