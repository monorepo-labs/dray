...
**Two way to write theme.** Derived = `--hue` + `--chroma`, fixed lightness per rung times per-step multiplier, so single-hue theme cost two numbers; `--chroma: 0` collapse every `calc()`, which why `default` = exactly greys it always was. **Ported theme not one hue**, so `catppuccin`, `gruvbox`, `one-dark-pro` and `cobalt2` name their rungs outright with palette's own token name in comment beside each. Both route end at same alias.
