...
**Frontend cache = 60s window, not permanent, and that correctness not tuning.** Rust re-scan every call *precisely* so editor installed while Dray run appear without restart; frontend cache that never expire take guarantee straight back — new app stay missing and moved bundle keep stale path on offer for life of process. Window = same one `useIssues` count read fresh for, against same two cost: panel remount on every session switch so clicking between session must be free, while installing editor and coming back must work. Failed read **change nothing** — no list written, no stamp — same bargain `usePrMarks` make.
