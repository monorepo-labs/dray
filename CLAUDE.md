...
**Window alone do nothing — menu opening = what re-check it.** Button mounted **forever** (panel hide, don't unmount), so mount effect run once for life of app and window it never re-check expire with nothing noticing — first version of this shipped exactly that, and window bought nothing. `refresh` therefore callable and hung off `onOpenChange`: list only have to be current when somebody about to read it. No poll and no visibility listener — app being installed = rare, and reader about to pick from menu = one event that care.
