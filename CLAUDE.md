...
- **Attachment = two features, because two kinds travel two ways.** [attachments.rs](apps/desktop/src-tauri/src/attachments.rs) decide which from extension _and_ size together, so composer's tile and wire can't disagree. **Image** — png/jpeg/gif/webp under 5MB, what API accept — become base64 `image` content block alongside prompt on same stdin line. Anything else become `@/abs/path` mention appended to prompt text, so non-image attachment need no wire surface at all: it prompt text by time it leave Rust, and CLI read file itself. Both verified live. Cheapness of second route = point — 40MB CSV cost path rather than context window — and also why oversized or unsupported image _degrade_ to mention instead of failing send.
  ...
  **Queued prompt carry paths, so composer hand attachment over instead.** Resolution happen at flush — that what let cancel give back exactly what was typed — so `QueuedMessage.attachmentPaths` = raw path and nothing drawable. Row therefore drawn from `byQueuedMessage`, written by `handleSendMsg` moment backend answer `queued`, keyed by that message's id. `send_msg` = only producer (relayed `dray send` pass `&[]`), so no queued prompt exist this cannot describe. **Nothing re-read, nothing async, nothing to go stale**: row draw exact attachment tray held.

  **Bounded by queue and deliberately by nothing else.** `retainQueuedAttachments` prune against live queue from **one** effect over `queuedBySession` — delivery, cancel, dropped queue and deleted session = four writes that shrink it, and release stated four time = three chance to miss one. First version instead cached every described path under byte budget, which = cap on wrong thing: eviction could drop attachment row still drawing, and refetching it evict another of *same message's* images, so prompt carrying more than budget could never draw all of it. Entry now live exactly as long as something can draw it.
  ...
  **Cancel restore in `handleCancelQueued`, synchronously, before its state write.** Esc restore text at once, so Enter pressed straight after would otherwise send that sentence without its file — and file then land in tray attached to whatever get typed next. Restore live there and not in composer for second reason: entry pruned against live queue, so reading it *after* that write read something already released.
