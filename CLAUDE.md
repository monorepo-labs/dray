...
**Two thing stop that read, and both keep it off ordinary prose.** Match whose last segment hold dot = already name file, so complete — `Updated /a/b/x.ts and src/foo.ts` keep its link rather than lose it to relative path four word later. Dot enough because of *where* truncation fall: scan cut at space, and piece before space inside path = directory name, never something ending `.ts`. And word *starting* with slash open path of own, which tell `open /a/b now and see /c/d` from break. What left not conclusive: `see /a/b and/or c` read as truncated and isn't, so that link lost — losing link = side to be wrong on.
