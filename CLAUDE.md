...
- **Codex run, over `codex app-server`, and it partial.** Session spawn, stream, resume and Stop; transcript draw message, reasoning, shell call and file diff; context ring fill. **Approval not wired** — Codex request get explicit `decline`, so session in asking stance (`auto`, `manual`, `plan`) have command refused, while `dontAsk`/`bypassPermissions` run whole. Worktree and fork refuse outright rather than run somewhere narrower. Subagent, MCP tool, web search, image prompt, mid-session model/mode change all absent. **Read [CODEX.md](apps/desktop/CODEX.md) first** — plain-language, and it carry the two thing that bite: `tokenUsage.last` = occupancy where `total` = cumulative (same trap `result.usage` set), and stale `codex` on PATH with no `app-server` subcommand answer nothing for 30s. Deep design, rejected alternative and staging = [CODEX-PLAN.md](apps/desktop/CODEX-PLAN.md).
