...
**Session's `cwd` = second, and it exist so read can happen where button isn't.** Deadlock without it: first read that fail leave no app, no app = no button, no button = no menu, and menu was only other trigger — so control gone until restart over one failed IPC call. Keyed on `cwd` it heal on next session switch instead, and cost nothing ordinarily since `load` no-op inside window. Rule worth carrying: **refresh trigger living only inside conditional render is one the failure case can delete.**
