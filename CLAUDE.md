...
**Sandbox refusing connect = own answer, never "app not running".** macOS seatbelt deny unix-socket connect with `EPERM`, and Dray spawn Codex into `workspace-write` by default (`approval_for`) — so agent's own `dray new` get denied on app's *own* default path, and CLI's blanket `map_err` reported user's running app as closed. `connect_failure` split it: socket `0600` owned by same user running both half, so `EPERM` on it = sandbox and nothing else, where closed app leave `ENOENT` (no socket file) or `ECONNREFUSED` (file, nobody behind). Every other errno keep closed-app sentence, so errno nobody anticipated under-report rather than blame sandbox never involved. Cure = retry that **one** command with escalated permissions, and it named in three place — error itself, [SKILL.md](apps/cli/skill/SKILL.md), [developer_instructions.md](apps/desktop/src-tauri/src/harness/codex/developer_instructions.md) — all pinned by test, since error reworded alone leave both doc answering sentence nothing emit any more. Test match on words not on line wrap: prose, so where author break line = not test's business.
