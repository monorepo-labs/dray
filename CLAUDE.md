...
**Frontend cache = 60s window, not permanent, and that correctness not tuning.** Rust re-scan every call *precisely* so editor installed while Dray run appear without restart; frontend cache that never expire take guarantee straight back — new app stay missing and moved bundle keep stale path on offer for life of process. Window = same one `useIssues` count read fresh for. Failed read **change nothing** — no list written, no stamp — same bargain `usePrMarks` make, and what stop blip certifying empty list fresh for a minute.

**Window alone do nothing — two trigger re-check it, and both needed.** Button mounted **forever** (panel hide, don't unmount), so mount effect run once for life of app and window nothing re-check expire with nothing noticing — first version shipped exactly that, and window bought nothing.

**Menu opening = first trigger.** List only have to be current when somebody about to read it. No poll and no visibility listener — app being installed = rare, and reader about to pick from menu = one event that care.

**Session's `cwd` = second, and it exist so read can happen where button isn't.** Deadlock without it: first read that fail leave no app, no app = no button, no button = no menu, and menu was only other trigger — so control gone until restart over one failed IPC call. Keyed on `cwd` it heal on next session switch, and cost nothing ordinarily since `load` no-op inside window. Rule worth carrying: **refresh trigger living only inside conditional render is one the failure case can delete.**

**Third = one retry, for reader who stay in session they in.** `cwd` key heal nothing where nothing change, so failed read get single retry after 4s — `usePrMarks`' own rule, and same reasoning: only read whose failure cannot correct itself earn one, and exactly one, since control that cannot be drawn must not become standing spawn. Gated on `failed` flag not on empty list: scan genuinely finding nothing = answer, and retrying it = waste.

**Failed launch force its tooltip open.** Radix close tooltip on click, so after failed press reader left with red glyph and no sentence unless they move away and hover back — glyph say something went wrong, `open`'s sentence = only thing saying what to do. `open={error ? true : undefined}`, so hover behave normally rest of time. Difference between *reporting* error and *filing* it.

**Failed launch reach reader, on button itself.** `open_in_app` answer `open`'s own sentence, and it only thing naming cure — bundle that moved, directory that gone. Console alone leave button looking simply unresponsive. So `open` return message rather than swallow it, button take alert glyph and destructive colour for 4s, sentence go in tooltip with `text-wrap` — those run long and truncating one clip exactly where cure start, same reason failed turn's notice wrap. **Word stay "Open"**: swapping label too make failure read as different control having appeared.
